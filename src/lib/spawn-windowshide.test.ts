...
// from v0.5.3) calls `child_process.spawn` or `spawnSync` to run a
// console-subsystem child (powershell.exe, where.exe, tasklist.exe,
// git.exe, ffmpeg.exe, bun.exe, ...), Windows allocates a fresh console
// window for that child because the parent has none to inherit. Result:
// the dashboard's 5-second doctor poll flashes 5+ console windows every
// tick — looks like a virus.
//
// Setting `windowsHide: true` in the options sets STARTF_USESHOWWINDOW
// + SW_HIDE so the allocated console stays invisible.
//
// This test is a hard lint that every spawn/spawnSync call in production
// source (src/**/*.ts excluding *.test.ts) declares `windowsHide`
// literally in its options bag. Catches the next time someone adds a
// spawn site and forgets the flag — without this we'd ship another
// v0.5.3-class regression. Verified 2026-05-05.
...
// Identify files that actually import spawn/spawnSync from
// node:child_process. Without this gate we'd false-positive on `spawn`
// as a method name (e.g. AgentProvider.spawn) in unrelated files.
function importsChildProcessSpawn(text: string): boolean {
  return /from\s+['"]node:child_process['"]/.test(text)
    || /from\s+['"]child_process['"]/.test(text)
    || /require\(['"]node:child_process['"]\)/.test(text)
...
describe('spawn/spawnSync windowsHide:true lint (subsystem=2 daemon prerequisite)', () => {
  it('every production spawn/spawnSync sets windowsHide in its options', () => {
    ...
    expect(violations, `${violations.length} spawn/spawnSync site(s) missing windowsHide:\n  ${violations.join('\n  ')}`).toEqual([])
  })

  // Sanity: the lint scanner actually finds spawn/spawnSync sites.
  // Without this, a refactor that moves all spawn calls into a missed
  // directory would pass the test for the wrong reason.
  it('lint scanner finds the known spawn/spawnSync sites (regression guard for the guard)', () => {
    let total = 0
    for (const file of scanFiles()) {
      const text = readFileSync(file, 'utf8')
      if (!importsChildProcessSpawn(text)) continue
