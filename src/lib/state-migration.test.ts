...
describe('full state-dir migration — upgrading-user smoke', () => {
  ...
  it('opens a fresh db with PRAGMA user_version = 19 and the 17 tables', () => {
    ...
    expect(tables.map(t => t.name)).toEqual([
      ...
      'milestones', 'observations', 'session_state', 'sessions', 'social_echo', 'social_seek', 'thread_extract_state', 'threads', 'turn_records',
    ])
  })
