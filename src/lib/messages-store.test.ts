...
import { makeMessagesStore, inboundMessageId, inboundFallbackMessageId, countMessagesSync } from './messages-store'
...
describe('messages store', () => {
  ...
  // countMessagesSync — onboarding-curiosity design §2 gate input
  it('countMessagesSync returns 0 for an unknown chat', () => {
    const db = openTestDb()
    expect(countMessagesSync(db, 'nonexistent')).toBe(0)
  })

  it('countMessagesSync counts both inbound and outbound messages', async () => {
    const db = openTestDb()
    ...
    await s.append({ id: '3', chatId: 'c1', ts: '2026-06-11T00:00:02Z', direction: 'in', kind: 'text', text: 'again', source: 'live' })
    expect(countMessagesSync(db, 'c1')).toBe(3)
  })

  it('countMessagesSync isolates counts per chat', async () => {
    const db = openTestDb()
    const s = makeMessagesStore(db)
    ...
    await s.append({ id: '3', chatId: 'chatB', ts: '2026-06-11T00:00:01Z', direction: 'in', kind: 'text', text: 'hi again', source: 'live' })
    expect(countMessagesSync(db, 'chatA')).toBe(1)
    expect(countMessagesSync(db, 'chatB')).toBe(2)
  })
})
