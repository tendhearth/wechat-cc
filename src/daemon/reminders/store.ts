 ...
 * daemon's SQLite db (migration v15).
 *
...
const COLS = 'id, chat_id, due_at, text, created_at, status, attempts, last_error'
...
export function makeRemindersStore(db: Db): RemindersStore {
  ...
  const stmtRecordAttempt = db.query<unknown, [string, string]>(
  ...
  return {
    ...
    async recordAttempt(id, error) {
      stmtRecordAttempt.run(error.slice(0, 500), id)
    },
  }
}
