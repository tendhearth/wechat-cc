...
describe('schema lookup tables', () => {
  it('REQUEST_SCHEMAS has 28 entries (POST body + 1 GET query)', () => {
    // 19 original + 4 a2a dashboard routes (preview, install, remove, pause)
    // + 1 a2a server-side test route (Test button) + 1 memory/delete
    // + 1 companion/import-local + 2 plugins (toggle, install)
    expect(Object.keys(REQUEST_SCHEMAS).length).toBe(28)
  })
