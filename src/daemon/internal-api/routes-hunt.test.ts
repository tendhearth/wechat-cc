...
let routes: ReturnType<typeof huntRoutes>
beforeEach(() => {
  const db = openDb({ path: ':memory:' })
  const hunt = makeHuntStore(db)
  hunt.recordHunt({ chatId: 'owner', text: '看这个 https://a.com/x' })
  deps = { hunt } as unknown as InternalApiDeps
  routes = huntRoutes(deps)
})

describe('GET /v1/hunt', () => {
  it('列出战利品', async () => {
    const r = await routes['GET /v1/hunt']!(qs(), undefined)
  ...
  it('limit 有上界,且垃圾值回落到默认(不是 NaN LIMIT)', async () => {
    for (const q of ['limit=99999', 'limit=abc', 'limit=-3', '']) {
      const r = await routes['GET /v1/hunt']!(qs(q), undefined)
      expect(r.status).toBe(200)
    }
  })

  it('**没接 store → 503,不是空清单** —— 空清单会被读成「CC 什么都没打到」', async () => {
    const r = await huntRoutes({} as InternalApiDeps)['GET /v1/hunt']!(qs(), undefined)
    expect(r.status).toBe(503)
  })
})

describe('POST /v1/hunt/status', () => {
  const idOf = async () => {
    const r = await routes['GET /v1/hunt']!(qs(), undefined)
  ...
  it('改成 using', async () => {
    const r = await routes['POST /v1/hunt/status']!(qs(), { id: await idOf(), status: 'using' })
    expect(r.body).toEqual({ ok: true })
  })

  it('不认识的状态 → 400,并告诉调用方允许哪些', async () => {
    const r = await routes['POST /v1/hunt/status']!(qs(), { id: await idOf(), status: '在用' })
    expect(r.status).toBe(400)
    expect((r.body as { allowed: string[] }).allowed).toContain('using')
  })

  it('缺 id → 400', async () => {
    expect((await routes['POST /v1/hunt/status']!(qs(), { status: 'using' })).status).toBe(400)
  })

  it('**id 不存在 → ok:false,不是静默 200 成功**', async () => {
    const r = await routes['POST /v1/hunt/status']!(qs(), { id: 'gone', status: 'using' })
    expect(r.body).toEqual({ ok: false })
  })
})

describe('POST /v1/hunt/remove', () => {
  it('删掉后就不在清单里了', async () => {
    const list = await routes['GET /v1/hunt']!(qs(), undefined)
    const id = (list.body as { items: Array<{ id: string }> }).items[0]!.id
    expect((await routes['POST /v1/hunt/remove']!(qs(), { id })).body).toEqual({ ok: true })
    const after = await routes['GET /v1/hunt']!(qs(), undefined)
...
describe('接线', () => {
  it('**三个路由真的进了总路由表** —— 写了一个路由文件却忘了 spread 进去,是不会有任何报错的', () => {
    ...
    for (const r of ['GET /v1/hunt', 'POST /v1/hunt/status', 'POST /v1/hunt/remove']) {
