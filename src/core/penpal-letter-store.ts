...
export interface LetterRow {
  ...
  /** 信封 kind(v39)。'letter' = 主人写的真信,payload 为 null。 */
  kind: string
  /** 信封 payload 的 JSON 文本;letter 为 null。 */
  payload: string | null
}
export interface LetterStore {
  create(l: { id: string; channelId: string; direction: 'in' | 'out'; sealedCiphertext: string; nonce: string; tag: string; plaintext: string; kind?: string; payload?: unknown }): void
...
export function makeLetterStore(db: Db): LetterStore {
  const ins = db.query<unknown, [string, string, string, string, string, string, string, string, string, string | null]>(
    `INSERT INTO penpal_letter(id, channel_id, direction, sealed_ciphertext, nonce, tag, plaintext, created_at, kind, payload)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  )
  ...
  // 未读只数真信:伙伴之间的信封(串门等)不是主人的来信。
  const selUnread = db.query<{ channel_id: string; n: number }, []>(
    "SELECT channel_id, COUNT(*) AS n FROM penpal_letter WHERE direction='in' AND read_at IS NULL AND kind='letter' GROUP BY channel_id")
  ...
  return {
    create(l) {
      ins.run(l.id, l.channelId, l.direction, l.sealedCiphertext, l.nonce, l.tag, l.plaintext, new Date().toISOString(),
        l.kind ?? 'letter', l.payload === undefined || l.payload === null ? null : JSON.stringify(l.payload))
    },
