...
export interface HandoffInput {sourceTaskId:string;targetProviderId:string;purpose:'review'|'revision';request:string;artifacts:ArtifactSelection[];quote?:ReviewQuote;targetTaskId?:string}
...
export function validateHandoffInput(input:HandoffInput):HandoffInput {
 ...
 return {...input,request:input.request.trim(),artifacts:input.artifacts.map(a=>({...a})),...(input.quote?{quote:{...input.quote}}:{})}
}
...
export function handoffContext(input:HandoffInput,source:StoredTask,events:TaskEvent[],files:Array<{name:string;text:string;selection:ArtifactSelection}>){
 ...
 if(!files.length)add('本次未附加文件快照；请按上述文字范围检查，不能声称已经读过未提供的文件。')
