...
type EventRow=Omit<TaskEvent,'runId'|'activity'> & {runId:string|null;activityJson:string|null}
type VisibleEvent=Extract<AgentEvent,{kind:'text'|'tool_call'|'error'}>
const SELECT='SELECT id,task_id AS taskId,kind,text,created_at AS createdAt,source_id AS sourceId,run_id AS runId,activity_json AS activityJson FROM workbench_events'
...
function publicEvent({runId,activityJson,...row}:EventRow):TaskEvent {
  return {...row,...(runId?{runId}:{}),...(activityJson?{activity:JSON.parse(activityJson) as AgentActivity}:{})}
}
...
export function makeTimelineEvents(db:Db) {
  ...
  const addEvent=(id:string,kind:TaskEvent['kind'],text:string,sourceId:string|null=null,runId:string|null=null)=>Number(
    db.query('INSERT INTO workbench_events(task_id,kind,text,created_at,source_id,run_id) VALUES(?,?,?,?,?,?)')
      .run(id,kind,text.slice(0,40_000),Date.now(),sourceId,runId).lastInsertRowid)
