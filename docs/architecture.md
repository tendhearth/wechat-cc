...
|---|---|---|---|---|
| **D1** | **Fragmented person model** — `person_brief` (plugin) vs `_overview.md`/`_profile.json` (daemon) never merge; two extraction pipelines (`threads` vs `wxfacts`) with no shared vocab/join key | Store | The *product thesis* is "coherent knowledge of each person" — currently there are two half-views. Directly undercuts the moat | **v1 SHIPPED 2026-07-12** (`knowledge-distill.ts` + `knowledgeMemorySection`): the daemon distills the owner's plugin knowledge (obligations + key/neglected contacts) into `knowledge.md`, injected always-on beside `profile.md` — the objective + subjective halves now compose in every prompt (needs `default_chat_id`). **Remaining:** feed plugin data into the gated `synthesizeOverview` narrative; the chatId→contact-name join for per-contact enrichment; reconcile `threads` ↔ `wxfacts` |
...
| **D3** | **Turn-serialization invariant is convention, not structure** — 3 callers must each remember runExclusive + dispatch-vs-dispatchInner; chatroom exempt; reply-sink keys only on chatId (no reply-ownership); reply route guest-tier | Channel | The single most fragile seam; a future turn-entry author can silently reintroduce a race; isolation depends on the mutex, not the auth model | One `enterTurn(chatId)` entrypoint that owns the lock + sink lifecycle; make dispatchInner un-callable without it. A channel-neutral `Turn` type so the app stops impersonating WeChat |
...
| **D6** | **`HF_ENDPOINT=hf-mirror.com` forced in manifests is broken here** (LocalEntryNotFoundError) — breaks wxmedia/wxsearch model downloads; two coexisting download paths (HF vs modelscope) | Deploy | Silently leaves plugins NOT READY; already bit us this session | Make HF endpoint overridable (env/config), pick one download path, or detect-and-fallback |
