...
- **2026-09-21 实现偏差**:§A 的顺序反了——回合号必须在 `await reviewCapture` **之前**加一(旧回合的释放先在同一个 promise 上排了队,先醒);`beginTurn` 把还没截过的基线带进新回合(不然回合中间补一句话会让整条任务永远截不到差异);探测器里「落定过的回合」按没租约看(释放中被唤醒也算自动续作),待决权限 / 提问不吃掉落定(先收成果、租约等 `isReplied`),解决权限 / 回答问题时再跑一次落定;`returnReviewFiles` 在保留会话路径上返回 `Promise<LiveInput>`,路由回 `{ input }`;桌面「打回」的门从 `status !== 'running'` 改成 `phase === 'replied'`。
