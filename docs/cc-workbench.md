...
- **2026-09-21**:租约按回合计代(外部评审 2026-09-21 的 #1 #2 #6 #7,四条都用假 runtime 复现过)。每次续接开一个新回合(`turn`),租约、差异基线和「释放」都带回合号——上一回合截快照时主人续接了,旧回合醒来发现回合号不对就不再放租约,同目录的下一件事不会挤进来并写。回合落定改按**状态转移**判(前台空闲且没有后台子任务),不再只认 `result` 事件:最后一个后台子任务只发工具事件也能收成果、放租约。保留会话在租约放掉之后自己又开始干活(后台通知)⇒ 立刻重新申请租约;申请不到(目录已被别的任务持有)⇒ 结束这个会话并在事件里说明——能保证的是最多一个事件的窗口,而不是两件事持续并写。「打回修改」按会话状态分路:保留着会话且已答复 ⇒ 走实时补充输入(同一段打回文本,请求 id 由快照 + 文件 + 意见派生,重发不重投);正在写 ⇒ `workbench_busy`;已收工 ⇒ 原来的续接。排队原因多一种 `retained_turn`(前一件正在续接)。
