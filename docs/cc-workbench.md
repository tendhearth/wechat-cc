...
在「此刻」写下要求，点「交给 CC 做」，选择已有项目后进入「一起做」的草稿。已有项目沿用执行者；原草稿、附件和模型选择保留，新要求追加并标明来源。没有项目时先添加文件夹，要求仍保留。只有点任务原有的开始按钮才启动执行，取消选择不会清空聊天输入，也不会把私人聊天历史自动带进项目。

点 CC 头像打开「正在照看的事」，按需查看等你回应、正在进行和最近的任务，点击回到同一个任务记录。安静的保留会话与正在执行区分显示。面板关闭或页面进入后台时停止读取；旧记录、断线与列表截断会明确说明。待处理请求复用桌面已有的全局汇总，不另开通知通道。

手机 PWA 的「一件事」现在可以处理当前任务的单次权限、回答执行者的问题、继续补充要求、查看和下载已保存成果。它仍走配对设备授权和原有加密隧道，执行留在电脑。权限和回答绑定任务、运行与请求，过期或重复的操作不能作用于下一轮。补充以同一请求编号重试；恢复会话需要额外确认时，手机保留草稿并提示回桌面处理，不自动新开会话。

成果按保存版本分块传送，完整校验后才显示图片、纯文本预览或下载链接；HTML 不作为网页执行。详情内容过长、无法完整经过隧道时，明确要求回桌面查看，保留草稿并停用旧权限卡，不截断授权说明。手机上仍不新建项目、不提供原生推送，也不能在电脑离线时执行。验收区分真实浏览器、实际本地服务及加密隧道，与真人手机／原生执行者链路；详见本批记录。
...
| Muse 借鉴、陪伴交办与手机任务入口 | [2026-09-22 实现与验证](superpowers/reports/2026-09-22-cc-companion-task-entry.md) |
