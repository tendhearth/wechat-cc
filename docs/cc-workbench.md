...
- **2026-09-18**：对话侧的 Cursor 也改走 ACP（每会话一个常驻 cursor-agent acp，wechat / delegate MCP 按会话注入并带逐会话 token 与 tier，主人聊天拿得到 admin 工具）；不再往 ~/.cursor/mcp.json 写静态钥匙，boot 时清掉上一版留下的条目；一次性评估（cheapEval）仍走 print 模式。设计：docs/superpowers/specs/2026-09-18-acp-cursor-chat-design.md。
