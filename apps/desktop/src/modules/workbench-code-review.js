...
export function renderWorkbenchCodeReview(source) {
  ...
  const report = value
  let limited = report.files.length > MAX_FILES || report.notes.length > 40 || report.preexistingPaths.length > MAX_FILES
  let charsLeft = MAX_DIFF_CHARS
  let linesLeft = MAX_LINES
  /** @param {string} text @param {number} [limit] */
  const label = (text, limit = 1000) => {
    if (text.length > limit) limited = true
  ...
  const renderDiff = diff => {
    const rows = []
    let offset = 0
    while (offset < diff.length && charsLeft > 0 && linesLeft > 0) {
      const newline = diff.indexOf('\n', offset)
      const end = newline < 0 ? diff.length : newline
      const length = end - offset
      const shown = Math.min(length, charsLeft, 4000)
      ...
      rows.push(`<span data-line="${kind}">${escape(line)}${truncated ? '…' : ''}</span>`)
      charsLeft -= shown + 1
      linesLeft--
      offset = newline < 0 ? diff.length : newline + 1
    }
    if (offset < diff.length) limited = true
  ...
  const files = report.files.slice(0, MAX_FILES).map((file, index) => {
    ...
    return `<details id="wb-review-file-${index}" class="wb-review-file" data-review-disclosure data-review-file="${index}"${index === 0 ? ' open' : ''}><summary><span class="wb-review-kind" data-kind="${file.kind}">${kind}</span><span class="wb-review-path">${label(file.path)}</span>${file.preexisting ? '<small>开始时已有修改</small>' : ''}</summary>${file.reason ? `<p class="wb-review-note">${label(file.reason, 2000)}</p>` : ''}${file.diff ? renderDiff(file.diff) : !file.reason ? '<p class="wb-review-note">这项记录没有可展开的文本差异。</p>' : ''}</details>`
  }).join('')
  ...
  return `<section class="wb-code-review"><header><h3>本轮开始与结束时的文件对比</h3><p class="wb-review-status" data-status="${report.status}">${status}${report.files.length ? ` · 报告列出 ${report.files.length} 个文件${skipped ? `，${skipped} 个未展开` : ''}` : ''}</p></header><p class="wb-review-note">外部程序在此期间的修改也会包含，不能据此认定修改来自哪一方。</p>${notes}${report.status === 'unavailable' && files ? `<p class="wb-review-note">${empty}</p>` : ''}${limited ? '<p class="wb-review-limit">预览已限量显示，请下载完整记录。报告中的检查范围与预览显示范围可能不同。</p>' : ''}${files || `<p class="wb-review-note">${empty}</p>`}${preexisting}${provenance}</section>${sourceDisclosure(source)}`
}
