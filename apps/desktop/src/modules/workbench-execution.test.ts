...
it('mirrors the daemon acp failure copy, including prefixed codes',async()=>{
 const {executionErrorMessage}=await import('./workbench-execution.js')
 expect(executionErrorMessage('acp_auth_required')).toContain('cursor-agent login')
 expect(executionErrorMessage('acp_session_failed: x')).toContain('acp')
 expect(executionErrorMessage('acp_process_exited: 1')).toContain('acp')
 ...
 expect(executionErrorMessage('acp_stop_refusal')).toContain('拒绝了这项要求')
 expect(executionErrorMessage('acp_turn_already_running')).toContain('等它答复')
 expect(executionErrorMessage('acp_attachments_unsupported')).toContain('移除附件')
 expect(executionErrorMessage('acp_unknown_code')).toBeNull()
})
